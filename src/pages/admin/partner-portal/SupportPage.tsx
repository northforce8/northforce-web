        ...
        <div className="grid grid-cols-1 md:grid-cols-4 gap-4">
          <div className="bg-white border border-gray-200 rounded-lg p-4">
            <div className="flex items-center justify-between mb-2">
              <span className="text-sm font-medium text-gray-600">Öppna Ärenden</span>
              <AlertCircle className="w-5 h-5 text-blue-600" />
            </div>
            <div className="text-3xl font-bold text-gray-900">{stats.open}</div>
            <div className="text-xs text-gray-500 mt-1">kräver uppmärksamhet</div>
          </div>

          <div className="bg-white border border-gray-200 rounded-lg p-4">
            <div className="flex items-center justify-between mb-2">
              <span className="text-sm font-medium text-gray-600">Pågående</span>
              <Clock className="w-5 h-5 text-yellow-600" />
            </div>
            <div className="text-3xl font-bold text-gray-900">{stats.inProgress}</div>
            <div className="text-xs text-gray-500 mt-1">bearbetas</div>
          </div>

          <div className="bg-white border border-gray-200 rounded-lg p-4">
            <div className="flex items-center justify-between mb-2">
              <span className="text-sm font-medium text-gray-600">Lösta</span>
          ...
          <div className="bg-white border border-gray-200 rounded-lg p-4">
            <div className="flex items-center justify-between mb-2">
              <span className="text-sm font-medium text-gray-600">SLA Bruten</span>
              <TrendingUp className="w-5 h-5 text-red-600" />
            </div>
            <div className="text-3xl font-bold text-gray-900">{stats.breached}</div>
            <div className="text-xs text-gray-500 mt-1">behöver eskalering</div>
          </div>
        </div>
        ...
        <div className="bg-white rounded-lg shadow">
          {filteredTickets.length === 0 ? (
            <div className="text-center py-12">
              ...
              <p className="text-gray-500 mb-2">Inga ärenden hittades</p>
              <button
                onClick={() => setShowCreateModal(true)}
                        ...
                        <div className="flex-1">
                          ...
                          <div className="flex flex-wrap items-center gap-4 text-xs text-gray-500">
                            <div className="flex items-center gap-1">
                              ...
                              <span>{customer?.company_name || 'Okänd'}</span>
                            </div>
                            <div className="flex items-center gap-1">
                    ...
                    {assignedPartner && (
                      <div className="ml-8 mt-2">
                        <div className="inline-flex items-center gap-2 px-3 py-1.5 bg-blue-50 text-blue-700 rounded-lg text-sm">
                          <User className="w-4 h-4" />
                          <span>Tilldelad: {assignedPartner.partner_name}</span>
                        </div>
                      </div>
                    )}
                    {ticket.resolved_at && (
                      <div className="ml-8 mt-2 text-sm text-gray-500">
                        Löst: {new Date(ticket.resolved_at).toLocaleDateString()}
                      </div>
                    )}
                  </div>
                );
        ...
        <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50 p-4">
          <div className="bg-white rounded-lg max-w-2xl w-full max-h-[90vh] overflow-y-auto">
            <div className="p-6 border-b border-gray-200">
              <h2 className="text-xl font-semibold text-gray-900">Skapa Supportärende</h2>
            </div>
